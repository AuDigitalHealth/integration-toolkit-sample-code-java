...
/**
 *	This sample class shows usage of the Pathology CDA Document creator to
 * produce a 3A level pathology document.
 *
 */
public class SamplePathologyCreation_3A {
	...
	public static void main(String[] args) throws ParserConfigurationException, JAXBException, SchematronValidationException {
	        ...
	        SubjectOfCareParticipant subject = new SubjectOfCareParticipantImpl( Arrays.asList( subjectIHI, medicareCard ), Arrays.asList(subjectOfCareAddress,subjectOfCareAddress), subjectOfCarePerson );
	        ...
	        EmploymentOrganisationImpl pathologyLabEmployment = new EmploymentOrganisationImpl( Arrays.asList( new HPIO("8003621231167886"), nataLabID ), "Pete's Pathologies" );
	        ...
	        AsEntityIdentifierImpl RequesterIdentifier = new AsEntityIdentifierImpl( "1.22.333.4444.55555", "12345678" );
	        ...
	        Occupation requesterOccupation = new OccupationImpl(  new ANZSCO_1ED_REV1( "599915", "Clinical Coder" ) );
	        ...
	        List<HPIO> organisationIds = Arrays.asList( new HPIO("8003621231167886")  );
	        ...
	        OrderDetails order = new OrderDetailsImpl( UniqueIdentifierImpl.createSimpleIdentifier( "2.16.840.1.113883.19.4.27", "10523475" ), new SNOMEDCode( "26604007", "Complete blood count" ) );

	        PathologyReportContext pathologyContext = new PathologyReportContextImpl( subject, requester, author, order );
	        ...
	        UniqueIdentifier doucmentID = new UniqueIdentifierImpl( "1.2.36.1.2001.1005.54.8003620833333789","10523477");
	        AttachedMedia testResult = new AttachedMedia( new File("test-result.pdf") );
	        String title = "Serum Report for Peter Patient";
	        ReportDocument testResultDocument = new ReportDocumentImpl( title, testResult,doucmentID, SNOMED_CT_ResultStatusCode.FINAL_RESULTS, now );
	        ...
	        PathologyResult pathologyResult2 = new PathologyResultImpl( 
	                new SNOMED_AU_Code( "166929004", "Glucose tolerance test" ), DiagnosticServiceCode.CHEMISTRY, now, specimenDetails2,SNOMED_CT_ResultStatusCode.FINAL_RESULTS);
	        List<PathologyResult> pathologyResults = Arrays.asList( pathologyResult1,pathologyResult2);
	        
	        PathologyReportContent pathologyContent = new PathologyReportContentImpl( pathologyResults, testResultDocument,reportingPathologist );

	        // Create Pathology Report
	        PathologyResultReport pathologyReport = new PathologyResultReportImpl( pathologyContext, pathologyContent );
	        ...
	        PathologyReportCdaModel model = new PathologyReportCdaModel( document, cdaLegalAuthenticator, cdaCustodian,  healthCareID, now );
	        PathologyReportCreator pathCreator = new PathologyReportCreator( model, pathologyReport );

	        //validate with schematron
	        //this is a useful tool for testing, it will validate against the
	        //default schematron. It's most likely too slow for a production system.
	        //this will use the default location for schematrons (resources/PathologyReport) at the 3b level
