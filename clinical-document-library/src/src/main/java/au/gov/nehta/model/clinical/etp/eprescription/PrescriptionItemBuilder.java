...
public class PrescriptionItemBuilder {
    ...
    /**
     * Add a State Authority Number <br>
     * may be chained with other calls
     */
    ...
    /**
     * Add a close the gap benifit id <br>
     * may be chained with other calls
     * <p>
     * 
     * Indicates that a subsidy under the PBS Closing the Gap Co-payment measure
     * applies to this item.
     * 
     * @param ctgBenefitID
     *            An extension and root detailing the CTG benefit identifiers
     */
    ...
    /**
     * THis Element Should NOT be populated if: (i) 1 the Medical Benefit
     * Category type is PBS; and 2 the item is not listed as ‘Authority
     * Required’, 'Authority Required (Streamlined)', or an application to
     * increase the maximum quantity and/or repeats has not been approved;
     * or(ii) the Medical Benefit Category type is neither PBS nor RPBS.
     * 
     * 
     * 
     * @param pbsAAN
     *            thePbs Authority Approval Number as a root+extension
     */
     ...
     * 
     * @param prn
     *            - An indicator that states the timing of the use or
    ...
    /**
     * Add an indicator describing if this item is for long term usage<br>
     * may be chained with other calls
     */
    ...
    /**
     * Add an medication instruction identifier<br>
     * may be chained with other calls
     */
    ...
    /**
     * Add a Streamlined Authority Approval Number<br>
     * may be chained with other calls
     */
    ...
    /**
     * Add an Ingrediten<br> may be called multiple times <br>
     * may be chained with other calls
     */
    ...
    private boolean isValid() throws IllegalArgumentException {
        ...
        ArgumentUtils.checkNotNull( item.getGroundsForConcurrentSupply(), "Prescripiton Item - groundsForConcurrentSupply" );
        ...
        ArgumentUtils.checkNotNull( item.getCloseTheGapBenifitId(), "Prescripiton Item - close the gap id" );
        return true;
    }
     ...
     * @throws IllegalArgumentException
     *             - if a mandatory data item has not been set.
     *             - if a supplied field is not allowed, eg: ReasonForTheraputicGood
     */
    ...
    private void isValidPrescriptionRequest() {
        ArgumentUtils.checkNotNull( item.getDateTimePrescriptionExpires(), "Prescripiton (Request) Item - Prescription expiry time " );
        ArgumentUtils.checkNotNull( item.getDateTimePrescriptionWritten(), "Prescripiton (Request) Item - Prescription written time " );
        ArgumentUtils.checkNotNull( item.getPbsRpbsItemCode(), "Prescripiton (Request) Item - therapeuticGoodIdentification pbs/rpbs item code" );
        ArgumentUtils.checkNotNull( item.getPrescriptionItemIdentifier(), "Prescripiton (Request) Item - prescriptionItemIdentifier" );
        ArgumentUtils.checkNotNull( item.getBrandSubstituteAllowed(), "Prescripiton (Request) Item - brandSubstituteAllowed" );
        ArgumentUtils.checkNotNull( item.getMaximumNumberOfRepeats(), "Prescripiton (Request) Item - maximumNumberOfRepeats" );
        ArgumentUtils.checkNotNull( item.getMedicalBenefitCategoryType(), "Prescripiton (Request) Item - medicalBenefitCategoryType" );
//        ArgumentUtils.checkNotNull( item.getGroundsForConcurrentSupply(), "groundsForConcurrentSupply" );

        ArgumentUtils.checkNotNull( item.getDirections(), "Prescripiton (Request) Item - instructions for use (diretions)" );
        ArgumentUtils.checkNotNull( item.getPbsType(), "Prescripiton (Request) Item - Pbs type" );
        ArgumentUtils.checkNotNull( item.getCloseTheGapBenifitId(), "Prescripiton (Request) Item - close the gap id" );
        
        
        if(item.getGroundsForConcurrentSupply() != null){
            throw new IllegalArgumentException("Grounds for concurrent supply is not allowed in a 'Prescription Request' prescription item ");
        }
        
        if(item.getReasonForTherapeuticGood() != null){
            throw new IllegalArgumentException("Reason for theraputic good is not allowed in a 'Prescription Request' prescription item ");
        }
        
        if(item.getMinimumIntervalBetweenRepeats() != null){
            throw new IllegalArgumentException("Minimum interval between repeats is not allowed in a 'Prescription Request' prescription item ");
        }
