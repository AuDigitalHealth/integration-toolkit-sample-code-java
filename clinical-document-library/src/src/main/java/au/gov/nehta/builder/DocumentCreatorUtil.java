...
public class DocumentCreatorUtil {
    ...
    /**
     * Turn a code into a string representation.
     * this impl ignores translations
     */
    public static String print( Coded code ){
    	...
    	if(code.isCoded() && code.getDisplayName() != null) return  code.getDisplayName();
    	
    	//else no orignal text and no display name, so use the code
    	if(code.isCoded() ) return  code.getCode();
	...
	public static JAXBElement<StrucDocContent> link(String href, String title) {
	    ...
	    JAXBElement<StrucDocLinkHtml> jaxbLinkHtml = new ObjectFactory().createStrucDocContentLinkHtml(link );
	...
	public static StrucDocTd printAuthorOrg(DocumentParticipant author) {
	...
	public static DateTime time(PreciseDate dateTimeStarted) {
		if(dateTimeStarted != null) return dateTimeStarted.getDateTime();
		return null;
	}

	public static DateTime time(PreciseDate start, PreciseDate end) {
		if(end != null) return end.getDateTime();
		if(start != null) return start.getDateTime();
	...
	public static String narrativeVersion(RestrictedTimeInterval i) {
		String timeInterval = "";
		if(i == null )return  timeInterval;
		
		//use the narrative form if we have it
		if(i.getNarrative() != null) return timeInterval = i.getNarrative();
		
		//else build up the time
		
		//high low
		if(i.getHigh() != null && i.getLow() != null) return print(i.getLow()) +" -> "+print(i.getHigh());
		
		//center width
		if(i.getCenter() != null && i.getWidth() != null) return print(i.getCenter()) +" ("+print(i.getWidth()) +")";
		
		
		//low only
		if(i.getLow() != null ) return print(i.getLow());
		
		//PW: 9/5/16 Added High only option which was missing
		//high only
		if(i.getHigh() != null ) return print(i.getHigh());

		
		//center only
		if(i.getCenter() != null ) return print(i.getCenter());
	...
	public static String display(PreciseDate start, PreciseDate end) {
		...
		if(start != null){
			...
			if(end == null) return buff.toString();
