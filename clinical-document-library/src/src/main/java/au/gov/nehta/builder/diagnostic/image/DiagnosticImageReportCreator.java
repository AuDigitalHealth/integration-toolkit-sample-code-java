...
public class DiagnosticImageReportCreator extends ClinicalDocumentCreator {
    ...
    public DiagnosticImageReportCreator( DiagnosticImagingReportCdaModel cdaModel, DiagnosticImageResultReport clinicalModel ) {
	...
	public Document create() throws ParserConfigurationException, JAXBException, SchematronValidationException {
        ...
        clinicalDocument.setCompletionCode( Converter.convertToCECode( cdaModel.getDIClinicalDoucment().getCompletionCode() ) );
...
//
            ...
            author = HeaderUtil.createAuthorWithTime( authorParticipant.getAssignedAuthorId(), authorParticipation.getRole(), authorParticipant.getAddresses(),
                    authorParticipant.getElectronicCommunicationDetail(), authorParticipantPerson, this.cdaModel.getAuthorTime() );
//        }
        return author;
    }
    ...
    private POCDMT000040Author getReportingDiagnostician( DocumentParticipant documentAuthor ) {
        ...
        if (documentAuthor != null && documentAuthor.getParticipant() != null) {
            ...
            POCDMT000040Person authorParticipantPerson = ClinicalModelConverter.getAuthorPerson( documentAuthor.getParticipant() );
            author = HeaderUtil.createAuthorWithTime( documentAuthor.getParticipant().getAssignedAuthorId(), documentAuthor.getRole(), documentAuthor.getParticipant().getAddresses(), documentAuthor
                    .getParticipant().getElectronicCommunicationDetail(), authorParticipantPerson, this.cdaModel.getAuthorTime() );
    ...
    private POCDMT000040Component3 getDiagnosticImageSection() {
        ...
        AuthorParticipant reportingRadiologist = clinicalModel.getContent().getReportingRadiologist().getParticipant();
        ...
        for (ImagingExaminationResult result : imageResults) {
            ...
            imgSection.setText( DiagnosticImageReportCreatorUtil.getImageExaminationResultNarrativeBlock( result ) );
        ...
        diagnosticImageSection.setText( DiagnosticImageReportCreatorUtil.diagnosticImageSection( clinicalModel.getContent() ));
        diagnosticImageSection.setClassCode( ActClass.DOCSECT );
        diagnosticImageSection.setMoodCode( ActMood.EVN );

        // Reporting pathologist
        diagnosticImageSection.getAuthor().add( getReportingDiagnostician( content.getReportingRadiologist() ) );

        diagnosticImageSection.setId( Converter.getII( content.getId() ) );
        POCDMT000040Entry relatedDocument = EntryCreator.getDiagnosticImageDocumentEntry( content.getRelatedDocument() );
    ...
    private POCDMT000040Component3 getAdministrativeObservationsSection() {
        ...
        if (subjectOfCareEntitlement != null) {
            List<Coverage2> subjectOfCareEntitlements = ClinicalModelConverter.getSubjectOfCareEntitlements( subjectOfCare );
