...
public class ClinicalDocumentCreator {
    ...
    public void useStrict(){
        if(resource == null){
            throw new IllegalArgumentException("useStrict was requested but no schematron files were specified. Try useStrict(SchematronResource resource) instead ");
        }
     ...
     * This will throw SchematronValidation errors on invalid CDA documents
     * 
     * @param SchematronResource  - The file location of the schematron files used to check the document
     *                              eg: "resources/schematron/PCEHR-DispenseRecord/schematron/ccd-3B.sch" 
     */
    ...
    public void check(Document d) throws  SchematronValidationException{
        SchematronCheckResult check = Schematron.check( resource.getTemplatePath(), resource.getSchematron(), d);
        if(check.schematronErrors.size() > 0) throw new SchematronValidationException(check);
    }


	public void addStylesheet(String stylesheetName, Document doc) {
		if(stylesheetName != null){
			ProcessingInstruction pi = doc.createProcessingInstruction( "xml-stylesheet", "type=\"text/xsl\" href=\"" + stylesheetName + "\"" );
