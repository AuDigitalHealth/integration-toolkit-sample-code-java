...
public class SharedhealthSummaryUtil extends DocumentCreatorUtil{
    ...
    public static StrucDocText getAdverseReactionsNarrativeBlock(AdverseReactions ars) {
        ...
        for(AdverseReaction ar:ars.getAdverseReactions()){
        	...
        	List<StrucDocTd> cols = new ArrayList<StrucDocTd>();
	...
	public static StrucDocText getMedicationsNarrativeBlock(Medications meds) {
	...
	public static StrucDocText getImmunisationsNarrativeBlock(Immunisations immunisations) {
		
	        if(immunisations.getAdministeredImmunisations() == null  ||immunisations.getAdministeredImmunisations().size() == 0){
	        	StrucDocText text = new StrucDocText();
	...
	public static StrucDocText getMedicalHistoryNarrativeBlock(MedicalHistory medicalHistory) {
		StrucDocText text = new StrucDocText();
		if(medicalHistory == null) return text;
		...
		List<NarrativeTripple> tripples = getTripples(procedures,problemDiagnosis,medicalHistory.getOtherHistoryItems());
	...
	private static List<NarrativeTripple> getTripples(Procedures procedures, ProblemDiagnoses problemDiagnoses, List<UncatagorisedMedicalHistoryItem> otherHistoryItems) {
		List<NarrativeTripple> tripples = new ArrayList<NarrativeTripple>();
	...
	private static void addExclusionParagraph(ExclusionStatement es, String alternate, StrucDocText text) {
		...
		if(es== null || es.getExclusionStatement() != null){
			//just show the message as per the CUP requierment
		//	para.getContent().add( print(es.getExclusionStatement()) );
	   //	}else{
			para.getContent().add(alternate);
		}
		
		JAXBElement<StrucDocParagraph> paraJaxbElement = new ObjectFactory().createStrucDocTextParagraph( para );
