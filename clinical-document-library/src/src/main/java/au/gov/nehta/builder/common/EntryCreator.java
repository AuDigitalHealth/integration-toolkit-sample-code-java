...
import au.net.electronichealth.ns.cda._2_0.EntityClassRoot;
import au.net.electronichealth.ns.cda._2_0.EntityDeterminer;
import au.net.electronichealth.ns.cda._2_0.IVLINT;
import au.net.electronichealth.ns.cda._2_0.IVLPQ;
import au.net.electronichealth.ns.cda._2_0.IVLTS;
import au.net.electronichealth.ns.cda._2_0.IVXBINT;
import au.net.electronichealth.ns.cda._2_0.IVXBTS;
import au.net.electronichealth.ns.cda._2_0.IntegrityCheckAlgorithm;
...
import au.net.electronichealth.ns.cda._2_0.POCDMT000040Criterion;
...
import au.net.electronichealth.ns.cda._2_0.POCDMT000040ExternalAct;
...
import au.net.electronichealth.ns.cda._2_0.POCDMT000040Precondition;
import au.net.electronichealth.ns.cda._2_0.POCDMT000040Procedure;
import au.net.electronichealth.ns.cda._2_0.POCDMT000040Product;
...
import au.net.electronichealth.ns.ci.cda.extensions._3.Coverage;
import au.net.electronichealth.ns.ci.cda.extensions._3.ManufacturedMaterial;
import au.net.electronichealth.ns.ci.cda.extensions._3.Policy;
import au.net.electronichealth.ns.ci.cda.extensions._3.PolicyOrAccount;
import au.net.electronichealth.ns.ci.cda.extensions._3.Qualifications;
import au.net.electronichealth.ns.ci.cda.extensions._3.Subject1;
...
public class EntryCreator extends Creator {
	...
	public static POCDMT000040Entry createSubjectOfCareCalcAgeEntry( Boolean dobCalculatedFromAge ) {
		return  getBaseObservationEntry(SectionEntryCodeSet.DATE_OF_BIRTH_IS_CALCULATED_FROM_AGE ,CDATypeUtil.getBL( dobCalculatedFromAge ) );
    }

	// Subject of Care Date Of Birth Accuracy Entry
	public static POCDMT000040Entry createSubjectOfCareDateOfBirthAccuracyEntry( DateAccuracy BirthAccuracy ) {
		return  getBaseObservationEntry(SectionEntryCodeSet.DATE_OF_BIRTH_ACCURACY_INDICATOR , Converter.convertScsAccuracyIndicator( BirthAccuracy ));
    }
    ...
    public static POCDMT000040Entry createDeathDateAccuracyEntry( DateAccuracy accuracy ) {
        return  getBaseObservationEntry(SectionEntryCodeSet.DATE_OF_DEATH_ACCURACY_INDICATOR , Converter.convertScsAccuracyIndicator( accuracy ));
    }
    
    public static POCDMT000040Entry createDeathSourceCodeEntry( Coded sourceCode) {
        return  getBaseObservationEntry(SectionEntryCodeSet.SOURCE_OF_DEATH_CODE ,Converter.convertToCDCode( sourceCode ));
    }
    ...
    public static POCDMT000040Entry createMothersMaidenNameEntry( String maidenName ) {
        ...
        return  getBaseObservationEntry(SectionEntryCodeSet.MOTHERS_MAIDEN_NAME , Converter.getPersonName( pn ) );
    }
	...
	public static POCDMT000040Entry createSubjectOfCareAgeAccuracyEntry( Boolean ageAccuracy ) {
		return  getBaseObservationEntry(SectionEntryCodeSet.AGE_ACCURACY_INDICATOR ,CDATypeUtil.getBL( ageAccuracy ));
	}
	...
	public static POCDMT000040Entry createSubjectOfCareBirthPluralityEntry( Integer birthPlurality ) {
		return  getBaseObservationEntry(SectionEntryCodeSet.BIRTH_PLURALITY ,CDATypeUtil.getINT( birthPlurality ));
	}
	...
	public static POCDMT000040Entry getSubstanceAdministrationEntry( PrescriptionItem item ) {
		...
		if( item.getGroundsForConcurrentSupply() != null){
    		...
    		groundsPolicy.setCode( ConcurrentSupplyGroundsCodeSet.convert( item.getGroundsForConcurrentSupply() ) );
		...
		if(item.getDosage() != null){
			addDosageEntries( substanceAdministration, consumeableManufacturedProduct, item.getDosage() );
			 ...
			 if(item.getDosage().getDoseQuantity() != null && item.getDosage().getDoseQuantity().getCodedUnit() !=null){
		            manufacturedMaterial.setFormCode( Converter.convertToCDCode( item.getDosage().getDoseQuantity().getCodedUnit() ) );
		     }
		}
		...
		if(item.getStartCriterion() != null){
			substanceAdministration.getEntryRelationship().add( getStartCriterionEntryRelationship( item.getStartCriterion() ) );
		}
		...
		if(item.getStopCriterion() != null){
			substanceAdministration.getEntryRelationship().add( getStopCriterionEntryRelationship( item.getStartCriterion() ) );
		}
	...
	protected static POCDMT000040Reference getMedicationInstructionReference( UniqueIdentifier medicationInstructionIdentifier ) {
        ...
        NCTISDataComponent insCode = new NCTISDataComponent( "103.16444","Medication Instruction Identifier");
        act.setCode( Converter.convertToCDCode( insCode ) );
        
        return medIns;
    }

    protected static void addExpmoraneousIngredients( POCDMT000040Material manufacturedMaterial,  List<Ingredient> extemporaneousIngredients) {
	...
	protected static void addDosageEntries( POCDMT000040SubstanceAdministration substanceAdministration, POCDMT000040ManufacturedProduct manufacturedProduct, Dosage dosage ) {
		
		QuantityUnitDescription qty = dosage.getDoseQuantity();
		if(qty.getQuantityDescription() != null){
			substanceAdministration.getEntryRelationship().add( getQuantityDescriptionEntry( qty.getQuantityDescription() ) );
		}
	...
	protected static POCDMT000040EntryRelationship getStopCriterionEntryRelationship( PreciseDate startCriterion ) {
	...
	protected static POCDMT000040EntryRelationship getStartCriterionEntryRelationship( PreciseDate startCriterion ) {
	...
	public static POCDMT000040Entry getSubstanceAdministrationEntry( DispenseItem item ) {
	    ...
	    JAXBElement<IVXBINT> high = new JAXBElement<IVXBINT>(new QName("urn:hl7-org:v3", "high"), IVXBINT.class, repeatNumberHigh);
		...
		if( item.getPatientCategory() != null  ){
		    substanceAdministration.getEntryRelationship().add( getPatientCategoryEntryRelationship( item.getPatientCategory() ) );
        }

		if( item.getRacfId() != null && item.getRacfId().length() > 0  ){
		    substanceAdministration.getEntryRelationship().add( getRACFIDEntryRelationship( item.getRacfId() ));
	     }
	...
	public static POCDMT000040Entry getExpiryEntry( PrescriptionItem scsPrescriptionItem ) {
		...
		observation.setEffectiveTime( Converter.convertScsDateToIVLTSWithoutTime( scsPrescriptionItem.getDateTimePrescriptionExpires() ) );
	...
	public static POCDMT000040EntryRelationship getPatientCategoryEntryRelationship( Coded patientCategory ) {
        POCDMT000040EntryRelationship entryRelationship = getCompER();
	...
	public static POCDMT000040EntryRelationship getATS4888SupplyEntryRelationship( PrescriptionItem item ) {
	...
	protected static void addDispensingQuantity( POCDMT000040Supply supply,  UniqueIdentifier pbsRpbsManufacturerCode,  QuantityUnitDescription  dispenseQuantity ) {
	     POCDMT000040ManufacturedProduct manufacturedProduct = addDispenseQuantity( supply, dispenseQuantity);
    ...
    protected static POCDMT000040ManufacturedProduct addDispenseQuantity( POCDMT000040Supply supply, QuantityUnitDescription dispenseQuantity ) {

        POCDMT000040ManufacturedProduct manuprod = null;

        // TODO code needs to be fixed to add a text description

        if( ( dispenseQuantity != null ) && (dispenseQuantity.getRealQuantity() !=null || dispenseQuantity.getCodedUnit() !=  null )){
			PQ pq = new PQ();
	...
	public static List<Coverage> getBenefitsCoverage( PrescriptionItem item ) {
		...
		List<Coverage> coverages = new ArrayList<Coverage>(5);
		...
		if(medicalBenefitCategoryType != null){
    		...
    		policyOrAccount.setCode( MedicalBenefitCategoryTypeCodeSet.convert( medicalBenefitCategoryType ) );
		...
		if(item.getPbsAuthorityApprovalNumber() != null ){
			...
			NCTISDataComponent pbsAan = new NCTISDataComponent( "103.10159", "PBS/RPBS Authority Approval Number" );
	...
	public static Coverage getEarlySupplyWithPharmaceuticalBenefitCoverage( DispenseItem dispenseItem ) {
		Coverage result = getBaseCoverage();
		PolicyOrAccount policyOrAccount = getCovPermPolicyOrAccount();
...
//		PrescriberInstructionRecipientParticipant prescriberInstructionRecipientParticipant = prescriberInstructionRecipient.getParticipant();
        ...
        //prescription request needs an set of empty elements for Schema correctness
	...
	protected static POCDMT000040Participant2 getParticipant( PrescriberInstructionRecipientParticipation instructionRecipientParticipation, String prescriberInstructionRecipientParticipantId ) {
		...
		PrescriberInstructionRecipientParticipant participant = instructionRecipientParticipation.getParticipant();

		POCDMT000040ParticipantRole participantRole = new POCDMT000040ParticipantRole();
		participantRole.setCode( Converter.convertToCECode( instructionRecipientParticipation.getRole().getCode() ) );
		...
		playingEntity.getAsEntityIdentifier().add( Converter.convert( participant.getEntityIdentifiers() ) );
		playingEntity.getName().add( Converter.getPersonName( participant.getPerson().getPersonName() ) );
		playingEntity.setAdministrativeGenderCode( Converter.convert( participant.getPerson().getDemographicData().getSex() ) );
		playingEntity.setBirthTime( CDATypeUtil.getTS( participant.getPerson().getDemographicData().getDateOfBirthDetail().getDateOfBirth() ) );
		...
		if (prescriberInstructionRecipientQualifications != null) {
			...
			qualificationCodeOriginalTextValue.getContent().add( prescriberInstructionRecipientQualifications );
		...
		if (participant.getElectronicCommunicationDetail() != null) {
			participantRole.getTelecom().addAll( Converter.convert( participant.getElectronicCommunicationDetail() ) );
		}
	...
	protected static POCDMT000040EntryRelationship getMediumEntryRelationship( PrescriberInstructionCommunicationMedium prescriberInstructionCommunicationMedium ) {
		...
		mediumEntryRelationshipObservation.getId().add( CDATypeUtil.getII( mediumEntryRelationshipObservationId ) );
		mediumEntryRelationshipObservation.setCode( SectionEntryCodeSet.PRESCRIBER_INSTRUCTION_COMMUNICATION_MEDIUM );
		mediumEntryRelationshipObservation.getValue().add( Converter.convertToCDCode( prescriberInstructionCommunicationMedium ) );
		mediumEntryRelationship.setObservation( mediumEntryRelationshipObservation );

		return mediumEntryRelationship;
	}

	protected static POCDMT000040EntryRelationship getSourceEntryRelationship( PrescriberInstructionSource prescriberInstructionSource ) {
		...
		sourceEntryRelationshipObservation.getId().add( CDATypeUtil.getII( sourceEntryRelationshipObservationId ) );
		sourceEntryRelationshipObservation.setCode( SectionEntryCodeSet.PRESCRIBER_INSTRUCTION_SOURCE );
		sourceEntryRelationshipObservation.getValue().add( Converter.convertToCDCode( prescriberInstructionSource ) );
    ...
    public static POCDMT000040Entry getLogoEntry( AttachedMedia logo ) {
        ...
        IntegrityCheckAlgorithm algorithm = IntegrityCheckAlgorithm.fromValue( logo.getIntegrityCheckType().getDigestName());
    ...
    private static POCDMT000040EntryRelationship getExaminationRequestDetails( ImagingExaminationResult diResult ) {
        ...
        act.setCode( Converter.convertToCDCode( new NCTISDataComponent( "102.16511", "Examination Request Details" ) ) );
	...
	private static POCDMT000040EntryRelationship getAnatomicalRegion(ImagingExaminationResult imageResult) {
		POCDMT000040EntryRelationship er = new POCDMT000040EntryRelationship();
		er.setTypeCode(XActRelationshipEntryRelationship.SUBJ);
		POCDMT000040Observation obs = getObsEvn();
		obs.setCode( Converter.convertToCDCode(new NCTISDataComponent("103.17009", "Anatomical Region") ));
		obs.getId().add(  Converter.getII( imageResult.getAnatomicalRegion().getID() ));
		obs.getValue().add(  Converter.convertToCDCode(imageResult.getAnatomicalRegion().getRegionCode()) );
	...
	private static POCDMT000040EntryRelationship getRelatedImage(ImagingExaminationResult diResult) {
		...
		relatedImagesAct.setCode(Converter.convertToCDCode(new NCTISDataComponent("102.16692", "Related Information")));
	...
	private static POCDMT000040EntryRelationship getResultStatus(ImagingExaminationResult imageResult) {
	...
	public static POCDMT000040EntryRelationship getImageTime( ImagingExaminationResult imageResult ) {
        ...
        ob.setCode(  Converter.convertToCDCode( new NCTISDataComponent( "103.16589", "Imaging Examination Result DateTime" ) ) );
    ...
    public static POCDMT000040EntryRelationship getExaminationProcedure( ImagingExaminationResult imageResult ) {
        POCDMT000040EntryRelationship er = new POCDMT000040EntryRelationship();
        er.setTypeCode( XActRelationshipEntryRelationship.REFR );
        
        POCDMT000040Act act = getActEvn();
        act.setCode( Converter.convertToCDCode( new NCTISDataComponent( "105.16633", "Examination Procedure" ) ) );
    ...
    protected static void addImagingAnatomicalSites( ImagingExaminationResult imageResult, POCDMT000040Observation ob ) {
        for(AnatomicalSite site: imageResult.getSites()){
            CD targetSiteCode = null;
            
            if( site.getSpecificLocationName() != null){
                ...
                targetSiteCode = Converter.convertToCDCode( CodeImpl.fromOriginalText( site.getDescription() ));
            }
    ...
    public static POCDMT000040EntryRelationship getTestResultStatus( PathologyResult pathologyResult ) {
          if( pathologyResult.getSpecimenDetail() == null) return null;
    ...
    public static POCDMT000040EntryRelationship getPathologyTestDate( PathologyResult pathologyResult ) {
        POCDMT000040EntryRelationship er = getERComp();
        POCDMT000040Observation ob = getObsEvn();
        ob.setCode( Converter.convertToCDCode( new NCTISDataComponent( "103.16605", "Pathology Test Result DateTime" )) );
    ...
    public static POCDMT000040EntryRelationship getDiagnosticService( PathologyResult pathologyResult ) {
        POCDMT000040EntryRelationship er = getERComp();
        POCDMT000040Observation ob = getObsEvn();
    ...
    public static POCDMT000040Entry getPathologyDocumentEntry( ReportDocument relatedDocument ) {
        ...
        JAXBElement<TEL> data = new JAXBElement<TEL>(new QName("urn:hl7-org:v3", "reference"), TEL.class, tel);
        ...
        low.setValue( Converter.HL7_DATE_TIME_FORMATTER.print( relatedDocument.getReportDate()) );
        ObjectFactory factory = new ObjectFactory();
	...
	public static POCDMT000040Entry getExclusionStatementEntry(String exclusionID, ExclusionStatement exclusionStatement) {
		POCDMT000040Entry er = new POCDMT000040Entry();
		POCDMT000040Observation obs = getObsEvn();
	...
	public static POCDMT000040Entry getMedicationsEntry(KnownMedication m) {
		...
		if(ci != null){
			...
			POCDMT000040Act act = getInfrmEvn();
			act.setCode( Converter.convertToCDCode( new NCTISDataComponent("103.10141", "Clinical Indication")) );
		...
		if(comment != null){
			...
			POCDMT000040Act act = getInfrmEvn();
			act.setCode( Converter.convertToCDCode(new NCTISDataComponent("103.16044", "Additional Comments")));
	...
	public static POCDMT000040Entry getProblemDiagnosisEntry(ProblemDiagnosis pd) {
		...
		obs.setCode(Converter.convertToCDCode( new SNOMED_AU_Code("282291009", "Diagnosis interpretation")));
		...
		if(pd.getDateOfRemission() != null){
			...
			remissionTime.setCode( Converter.convertToCDCode( new NCTISDataComponent("103.15510", "Date of Resolution/Remission") ) );
		...
		if(pd.getComment() != null){
			...
			POCDMT000040Act act = getInfrmEvn();
			act.setCode( Converter.convertToCDCode( new NCTISDataComponent("103.16545", "Problem/Diagnosis Comment") ) );
	...
	public static POCDMT000040Entry getProceduresEntry(Procedure p) {
		...
		if(p.getComment() != null){
			...
			act.setCode(Converter.convertToCDCode(new NCTISDataComponent("103.15595","Procedure Comment")));
	...
	public static POCDMT000040Entry getOtherMedicalHistoryItemEntry(UncatagorisedMedicalHistoryItem item) {
		POCDMT000040Entry e = new POCDMT000040Entry();
		POCDMT000040Act act = getActEvn();
		
		act.getId().add(Converter.getII(item.getID()));
		act.setCode(Converter.convertToCDCode( new NCTISDataComponent("102.16627", "Uncategorised Medical History Item")));
		...
		if(item.getComment() != null){
			...
			actevn.setCode(Converter.convertToCDCode(new NCTISDataComponent("103.16630", "Medical History Item Comment")));
