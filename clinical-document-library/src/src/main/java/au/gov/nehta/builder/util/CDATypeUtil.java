...
public class CDATypeUtil {
	private final static String HL7_DATE_FORMAT = "yyyyMMddHHmmssZ";
	private final static DateTimeFormatter HL7_DATE_FORMATTER = DateTimeFormat.forPattern( HL7_DATE_FORMAT );
	...
	public static II getII( String root, String extenstion ) {
        ArgumentUtils.checkNotNull( extenstion, "value" );
        II result = new II();
        result.setRoot( root );
        result.setExtension( extenstion );
        return result;
    }
	...
	public static CE getCE( String code, String codeSystem, String codeSystemName, String displayName ) {
		CE result = new CE();
		result.setCode( code );
	...
	public static CD getCD( String code, String codeSystem, String codeSystemName, String displayName ) {
		CD result = new CD();
		result.setCode( code );
		result.setCodeSystem( codeSystem );
		result.setCodeSystemName( codeSystemName );
	...
	public static CV getCV( String code, String codeSystem, String codeSystemName, String displayName ) {
        CV result = new CV();
        result.setCode( code );
        result.setCodeSystem( codeSystem );
        result.setCodeSystemName( codeSystemName );
   ...
   public static SC getSC( String value ) {
	...
	/**
	public static CD getCD( CodeableText codeableText ) {
		ArgumentUtils.checkNotNull( codeableText, "codeableText" );

		CD result = null;

		if (codeableText.isCoded()) {
			result = CDATypeUtil.getCD( codeableText.getCode(), codeableText.getCodeSystem(), codeableText.getCodeSystemName(), codeableText.getDisplayName() );
			result.setOriginalText( getED( codeableText.getOriginalText() ) );
		} else {
			result = new CD();
			result.setOriginalText( getED( codeableText.getUncodedValue() ) );
			result.setDisplayName( codeableText.getUncodedValue() );
		}

		return result;
	}

	public static CE getCE( CodeableText codeableText ) {
		ArgumentUtils.checkNotNull( codeableText, "codeableText" );
		CE result = null;

		if (codeableText.isCoded()) {
			result = CDATypeUtil.getCE( codeableText.getCode(), codeableText.getCodeSystem(), codeableText.getCodeSystemName(), codeableText.getDisplayName() );
			result.setOriginalText( getED( codeableText.getOriginalText() ) );
		} else {
			result = new CE();
			result.setOriginalText( getED( codeableText.getUncodedValue() ) );
