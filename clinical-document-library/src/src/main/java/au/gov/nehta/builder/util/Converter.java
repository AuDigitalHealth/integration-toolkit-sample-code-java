...
public class Converter {
	private final static String HL7_DATE_TIME_FORMAT = "yyyyMMddHHmmssZ";
	private final static String HL7_DATE_ONLY_FORMAT = "yyyyMMdd";

	//joda date time formatters are threadsafe
	public final static DateTimeFormatter HL7_DATE_TIME_FORMATTER = DateTimeFormat.forPattern( HL7_DATE_TIME_FORMAT );
	public final static DateTimeFormatter HL7_DATE_ONLY_FORMATTER = DateTimeFormat.forPattern( HL7_DATE_ONLY_FORMAT );
	...
	public static ON getOrganizationNameAndUse( Organisation scsOrganisation ) {
		...
		if (scsOrganisation.getOrganisationName() != null) {
			...
			if (scsOrganisation.getOrganisationNameUsage() != null) {
				organizationName.getUse().add( getEntityOrganisationNameUse( scsOrganisation.getOrganisationNameUsage() ) );
			}
		}
	...
	private static EntityNameUse getEntityOrganisationNameUse( OrganisationNameUsage scsOrganisationNameUsage ) {
	...
	public static List<TEL> convert( List<? extends Telecom> telecoms ) {
		if(telecoms == null) return new ArrayList<TEL>(1);
	...
	public static TEL convert( Telecom ecd ) {
		TEL telecom = new TEL();
		String telecomValuePrefix = null;
	...
	/**
	 * Special override for ATS 4888 Prescrptions that need URG_PQ where PCEHR Prescriptions need PQ only
	 * 
	...
	public static List<PN> convertNames(List<PersonName> names){
		...
		List<PN> pns = new ArrayList<PN>(names.size());
	...
	public static PN getPersonName( PersonName scsPersonName ) {
		...
		if (scsPersonName.getPreferredNameIndicator() != null && scsPersonName.getPreferredNameIndicator() == true) {
	...
	public static List<EntityIdentifier> getAsEntityIdentifier( List<? extends AsEntityIdentifier> ids ) {
		List<EntityIdentifier> ei = new ArrayList<EntityIdentifier>( 5 );
	...
	public static CE convertToCECode( Coded value ) {
        ...
        if(value.isCoded()){
         ce = CDATypeUtil.getCE( value.getCode(), value.getCodeSystem(), value.getCodeSystemName(), value.getDisplayName() );
	...
	public static CV convertToCVCode( Coded value ) {
	    CV cv= null;
        if(value.isCoded()){
         cv = CDATypeUtil.getCV( value.getCode(), value.getCodeSystem(), value.getCodeSystemName(), value.getDisplayName() );
         if(value.getOriginalText() != null){
	...
	public static ON getOrganizationName( OrganizationName cdaModelLegalAuthenticatorAssignedEntityRepresentedOrganizationName ) {
		ON organizationName = new ON();
		organizationName.getContent().add( cdaModelLegalAuthenticatorAssignedEntityRepresentedOrganizationName.getValue() );
	...
	public static  List<EntityIdentifier> convertEntityIdentifiers( List<AsEntityIdentifier> asEntityIdentifiers ) {
		if(asEntityIdentifiers == null) return new ArrayList<EntityIdentifier>();
		
		List<EntityIdentifier> ids = new ArrayList<EntityIdentifier>(asEntityIdentifiers.size());
	...
	public static EntityIdentifier convert( AsEntityIdentifier asEntityIdentifier ) {
		...
		if (null != asEntityIdentifier.getAssigningGeographicAreaName()) {
			...
			EntityClass entityClass = EntityClass.fromValue( asEntityIdentifier.getAssigningGeographicAreaClassCode() );
	...
	public static List<AD> convertAddresses(List<PostalAddress> addresses) {
		if(addresses==null) return new ArrayList<AD>(1);
		
		List<AD> lad = new ArrayList<AD>(addresses.size());
