...
public class StructuredBodyUtil {
	public static POCDMT000040Author getSectionAuthor( POCDMT000040Author clinicalDocumentAuthor, DateTime prescriptionCreationDate ) {
	...
	public static StrucDocText getAdministrativeObservationsNarrativeBlock( SubjectOfCareDemographicData demoData ) {
		...
		administrativeObservationsTableHead.getTr().add( NarrativeUtil.getTableHeaderRow( Arrays.asList( "Name", "Value" ) ) );
        ...
        if (dobDetail != null) {
            if (dobDetail.getDateOfBirthIsCalculatedFromAge() != null) {
                observationsTableBody.getTr().add(
                        NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Date Of Birth is Calculated from Age",
            ...
            if (dobDetail.getDateOfBirthAccuracyIndicator() != null) {
                observationsTableBody.getTr().add(
                        NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Date of Birth Accuracy Indicator", dobDetail.getDateOfBirthAccuracyIndicator().toString() ) ) );
            }
        ...
        if (demoData.getBirthPlurality() != null) {
            ...
            observationsTableBody.getTr().add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Birth Plurality", birthPlurality.toString() ) ) );
        }

        if (demoData.isAgeAccurate() != null) {
            observationsTableBody.getTr()
                    .add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Age Accuracy Indicator", NarrativeUtil.booleanToYesNo( demoData.isAgeAccurate() ) ) ) );
        }

        
        observationsTableBody.getTr().add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Age", Integer.toString(demoData.getAgeInYears()) ) ) );
        

		administrativeObservationsTable.getTbody().add( observationsTableBody );
		JAXBElement<StrucDocTable> administrativeObservationsTableJaxbElement = new ObjectFactory().createStrucDocTextTable( administrativeObservationsTable );
	...
	public static StrucDocText getAdministrativeObservationsNarrativeBlock( ExtendedDemographicData demoData ) {
        StrucDocText narrativeBlock = new StrucDocText();
        ...
        if( dateOfDeath != null ){
            ...
            if(accuracy != null){
                observationsTableBody.getTr()
                .add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Date of Death Accuracy Indicator", accuracy.toString() ) ) );
            }
            
            Coded source = dateOfDeath.getDeathNotificationSource();
            if(source != null){
                observationsTableBody.getTr()
                .add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Source Of Death Notification", source.getDisplayName() ) ) );
        ...
        if (demoData.getMothersOriginalFamilyName() != null) {
            observationsTableBody.getTr()
                    .add( NarrativeUtil.createTableBodyRowWithCellValues( Arrays.asList( "Mothers Original Family Name",demoData.getMothersOriginalFamilyName() ) ) );
        }
