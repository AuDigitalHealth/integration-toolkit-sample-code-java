...
/**
 * Create a Shared Health Summary CDA Document 
 * optionally checking with schematrons if .isStrict() returns true 
 */
public class SharedHealthSummaryCreator extends ClinicalDocumentCreator {
	...
	public SharedHealthSummaryCreator( SharedHealthSummaryCDAModel cdaModel, SharedHealthSummary clinicalModel  ) {
	...
	private POCDMT000040Component3 getImmunisations() {
		...
		if(immunisations == null) return null;
		...
		if(immunisations.getExclusionStatement() != null){
			section.getEntry().add(EntryCreator.getExclusionStatementEntry("103.16302.120.1.5",immunisations));
		}
	...
	private POCDMT000040Component3 getMedicalHistory() {
		...
		section.setCode(Converter.convertToCECode( new NCTISDataComponent("101.16117", MEDICAL_HISTORY)));
	...
	private void addOtherHistoryEntries(POCDMT000040Section section, List<UncatagorisedMedicalHistoryItem> otherHistoryItems) {
		if(otherHistoryItems == null) return;
	...
	private void addProcedureEntries(POCDMT000040Section section, Procedures procedures) {
		...
		if( procedures.getExclusionStatement() != null){
			section.getEntry().add(EntryCreator.getExclusionStatementEntry("103.16302.120.1.4",procedures));
		}
	...
	private void addProblemDiagnosisEntries(POCDMT000040Section section, ProblemDiagnoses problemDiagnosis) {
		if(problemDiagnosis == null)return;
		...
		if( problemDiagnosis.getExclusionStatement() != null){
			section.getEntry().add(EntryCreator.getExclusionStatementEntry("103.16302.120.1.3",problemDiagnosis));
		}
	...
	private POCDMT000040Component3 getAdverseReactions() {
		...
		if(adverseReactions == null) return null;
		
		
		POCDMT000040Component3 reactions = new POCDMT000040Component3();
		POCDMT000040Section section = new POCDMT000040Section();
		section.setCode(Converter.convertToCECode( new NCTISDataComponent("101.20113", ADVERSE_REACTIONS)));
		...
		if(adverseReactions.getExclusionStatement() != null){
			section.getEntry().add(EntryCreator.getExclusionStatementEntry("103.16302.120.1.1",adverseReactions));
		}
	...
	private POCDMT000040Component3 getMedications() {
		...
		POCDMT000040Component3 comp = new POCDMT000040Component3();
		POCDMT000040Section section = new POCDMT000040Section();
		section.setCode(Converter.convertToCECode( new NCTISDataComponent("101.16146", "Medication Orders")));
	...
	private StrucDocText getMedicationsNarrative() {
		return SharedhealthSummaryUtil.getMedicationsNarrativeBlock(clinicalModel.getContent().getMedications());
	}

	private StrucDocText getMedicalHistoryNarrative() {
		return SharedhealthSummaryUtil.getMedicalHistoryNarrativeBlock(clinicalModel.getContent().getMedicalHistory());
	}
	private StrucDocText getReactionNarrative() {
		return SharedhealthSummaryUtil.getAdverseReactionsNarrativeBlock(clinicalModel.getContent().getAdverseReactions());
	}
	
	private StrucDocText getImmunisationsNarrative() {
		return SharedhealthSummaryUtil.getImmunisationsNarrativeBlock(clinicalModel.getContent().getImmunisations());
	}
	...
	private POCDMT000040Author getDocumentAuthor() {
		...
		if(authorTime == null) authorTime = cdaModel.getLegalAuthenticator().getAuthenticationTime();
		...
		assignedAuthor.getTelecom().addAll(Converter.convert(documentAuthor.getElectronicCommunicationDetails()));
		...
		person.setAsEmployment( ClinicalModelConverter.getExtendedEmploymentOrganisation(employmentDetail));
	  ...
	  private POCDMT000040Component3 getAdministrativeObservationsSection() {
			...
			if (authorEntitlement != null) {
				List<Coverage2> authorEntitlements = ClinicalModelConverter
						.getParticpantEntitlements(authorEntitlement,author.getID());
				administrativeObservationsSection.getCoverage2().addAll(
						authorEntitlements);
			}
